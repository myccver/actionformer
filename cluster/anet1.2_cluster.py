...
with open('/home/yunchuan/actionformer/data/anet_1.2/annotations/anet1.2_fake_gt.json', 'rb') as f:
    fake_gt = json.load(f)['database']

train_list = ['v_'+vid for vid,data in fake_gt.items() if data['subset'] == 'training']

video_dict = {vid: data for vid, data in video_dict.items() if vid in train_list}


for vid, vid_data in video_dict.items():
    if "{}.npy".format(vid) not in os.listdir('/home/yunchuan/actionformer/data/anet_1.2/tsp_features'):
        continue
    feats = np.load('/home/yunchuan/actionformer/data/anet_1.2/tsp_features/{}.npy'.format(vid)).astype(np.float32)
...
# 创建保存目录（如果不存在则创建）
os.makedirs(f"./anet_1.2/{k_str}/cluster_centers", exist_ok=True)
os.makedirs(f"./anet_1.2/{k_str}/cluster_representatives", exist_ok=True)
...
for label, feature_list in label_dict.items():
    ...
    # 保存两个拼接结果
    np.save(os.path.join(f'./anet_1.2/{k_str}', f"cluster_centers/{label}.npy"), cluster_centers)
    np.save(os.path.join(f'./anet_1.2/{k_str}', f"cluster_representatives/{label}.npy"), representatives)

    # 打印索引
    print(f"\n【类 {label}】")
    print(f"每个簇中距离质心最近的样本索引： {closest_indices.tolist()}")
    print(f"→ 已保存: ./anet_1.2/{k_str}/cluster_centers/{label}.npy 和 ./anet_1.2/{k_str}/cluster_representatives/{label}.npy")
