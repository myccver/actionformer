...
for vid, vid_data in video_dict.items():
    if "{}.npy".format(vid) not in os.listdir('/home/yunchuan/actionformer/data/thumos/i3d_features'):
        continue
    feats = np.load('/home/yunchuan/actionformer/data/thumos/i3d_features/{}.npy'.format(vid)).astype(np.float32)

    # deal with downsampling (= increased feat stride)
    feats = feats[::1, :]
    feat_stride = 4 * 1
    feat_offset = 0.5 * 16 / feat_stride
    pass
    fps = gt[vid]['fps']
    for point_data in vid_data:
        if point_data['class'] not in label_dict:
            label_dict[point_data['class']] = []
        point_feat_index = int(point_data['point_time'] * fps / feat_stride - feat_offset)
...
# 创建保存目录（如果不存在则创建）
os.makedirs(f"./thumos/{k_str}/cluster_centers", exist_ok=True)
os.makedirs(f"./thumos/{k_str}/cluster_representatives", exist_ok=True)
...
for label, feature_list in label_dict.items():
    ...
    # 保存两个拼接结果
    np.save(os.path.join(f'./thumos/{k_str}', f"cluster_centers/{label}.npy"), cluster_centers)
    np.save(os.path.join(f'./thumos/{k_str}', f"cluster_representatives/{label}.npy"), representatives)

    # 打印索引
    print(f"\n【类 {label}】")
    print(f"每个簇中距离质心最近的样本索引： {closest_indices.tolist()}")
    print(f"→ 已保存: ./thumos/{k_str}/cluster_centers/{label}.npy 和 ./thumos/{k_str}/cluster_representatives/{label}.npy")
