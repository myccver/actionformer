...
def truncate_feats_with_points(
    data_dict,
    max_seq_len,
    trunc_thresh,
    offset,
    ...
    for _ in range(max_num_trials):
        ...
        right = torch.minimum(window[:, 1] + offset, data_dict['segments'][:, 1])
        # 处理点标注
        point_left = torch.maximum(window[:, 0] - offset, data_dict['points'][:])
        point_right = torch.minimum(window[:, 1] + offset, data_dict['points'][:])
        point_idx = (point_left == point_right)
        ...
        # if seg_idx.sum()==0:
        #     seg_idx = (inter_ratio >= 0)

        # # 合并seg_idx 和 point_idx, harm perfomance
        # seg_idx = torch.logical_and(seg_idx, point_idx)
    ...
    # 改变点标注相对位置
    data_dict['points'] = data_dict['points'][seg_idx] - st
    assert data_dict['segments'].shape[0]==data_dict['points'].shape[0]
    assert data_dict['segments'].shape[0] != 0
    assert data_dict['points'].shape[0]!=0
